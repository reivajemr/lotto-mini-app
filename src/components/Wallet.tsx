...
export default function Wallet({
  ...
  return (
    <div className="p-4 space-y-4">
      ...
      {/* ── Aviso móvil: usar Tonkeeper ─────────────────── */}
      {!isConnected && (
        <div className="bg-blue-500/10 border border-blue-500/30 rounded-xl p-3 flex items-start gap-2">
          <span className="text-lg flex-shrink-0">💡</span>
          <div>
            <p className="text-blue-200 text-xs font-medium">Recomendado: usar Tonkeeper</p>
            <p className="text-blue-200/60 text-xs mt-0.5">
              Al conectar, selecciona <strong>Tonkeeper</strong> para mejor compatibilidad en móvil y PC.
            </p>
          </div>
        </div>
      )}
          ...
          {isConnected ? (
            <div className="bg-green-500/10 border border-green-500/30 rounded-xl p-3 flex items-center gap-2">
              ...
              <p className="text-green-300 text-xs font-medium">
                Wallet conectada — elige un paquete y confirma
              </p>
            </div>
          ) : (
