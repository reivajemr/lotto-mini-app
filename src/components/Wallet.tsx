...
export default function Wallet({
  ...
  const sendDeposit = async (pack: typeof PACKS[0]) => {
    ...
    try {
      ...
      const transaction = {
        // ✅ validUntil en SEGUNDOS Unix, máximo 5 minutos
        validUntil: Math.floor(Date.now() / 1000) + 300,
        messages: [
          {
            address: adminWallet,
            amount: toNano(pack.ton),
            // Sin payload para evitar errores de validación
          },
        ],
      };
      ...
      if (result?.boc) {
        // 1. Registrar depósito en backend (obtenemos un depositId)
        ...
        if (regData?.depositId) {
          setPendingDepositId(regData.depositId);
          ...
          // Iniciar polling con el depositId
      ...
      if (
        ...
        !msg.includes('Reject') &&
        !msg.includes('No enough funds') // fondos insuficientes: el usuario ya ve el error en la wallet
      ) {
        showAlert('❌ Error: ' + msg);
      }
    } finally {
  ...
  const startPolling = useCallback((depositId: string, expectedLechugas: number) => {
    ...
    const poll = async () => {
      ...
      if (attempts < 24) setTimeout(poll, 5000); // 24 intentos × 5s = 2 min
    };
    setTimeout(poll, 8000); // esperar 8s antes del primer check
  }, [telegramId]);
    ...
    <div className="p-4 space-y-4">
      ...
      <div className="bg-gradient-to-br from-teal-600/30 to-emerald-600/20 border border-teal-500/30 rounded-2xl p-5">
        ...
        {isConnected ? (
          <div className="flex items-center justify-between bg-teal-500/20 rounded-xl px-3 py-2.5 border border-teal-500/40">
            ...
            <button onClick={() => tonConnectUI.disconnect()} className="text-white/30 hover:text-red-400 text-xs transition-colors px-2 py-1 rounded-lg hover:bg-red-400/10">
              ✕
            </button>
          </div>
        ) : (
      ...
      <div className="flex bg-white/5 rounded-xl p-1 gap-1">
        {(['deposit', 'withdraw'] as const).map(tab => (
          <button key={tab} onClick={() => { haptic('light'); setActiveTab(tab); }}
        ...
        <div className="space-y-4">
          ...
          {isConnected ? (
            <div className="bg-green-500/10 border border-green-500/30 rounded-xl p-3 flex items-center gap-2">
              ...
              <p className="text-green-300 text-xs font-medium">Wallet conectada — elige un paquete y confirma en tu app</p>
            </div>
          ) : (
            <div className="bg-blue-500/10 border border-blue-500/30 rounded-xl p-3 flex items-start gap-2">
              <span className="text-base">💎</span>
              <p className="text-blue-200 text-xs">Conecta tu TON Wallet para depósito automático.</p>
            </div>
          )}
                ...
                <div key={pack.lechugas} className={`rounded-xl border p-3.5 relative ${pack.popular ? 'bg-teal-500/15 border-teal-500/40' : 'bg-white/5 border-white/10'}`}>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {isConnected ? (
                        <button onClick={() => sendDeposit(pack)} disabled={txPending || depositPending}
            ...
            <div className="mt-3 space-y-3 pl-3 border-l border-white/8">
              <div className="bg-white/5 border border-white/10 rounded-xl p-3">
                ...
                {adminWallet ? (
                  <div className="flex items-center gap-2">
                    ...
                    <button onClick={() => copy(adminWallet, 'admin')} className="bg-white/10 px-2 py-1 rounded text-xs text-white">{copied === 'admin' ? '✅' : '📋'}</button>
                  </div>
              ...
              <div className="bg-white/5 border border-white/10 rounded-xl p-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button onClick={() => copy(telegramId, 'id')} className="bg-white/10 px-2 py-1 rounded text-xs text-white">{copied === 'id' ? '✅' : '📋'}</button>
                </div>
              </div>
            </div>
          </details>
      ...
      {activeTab === 'withdraw' && (
        <div className="space-y-4">
          ...
          <div className="bg-white/5 border border-white/10 rounded-2xl p-4 space-y-2">
            ...
            {['1️⃣ Ingresa el monto en TON','2️⃣ El admin verifica y envía en 24-48h','3️⃣ Confirmación por Telegram ✅'].map((s,i)=><p key={i} className="text-white/50 text-xs">{s}</p>)}
          ...
          {isConnected ? (
            ...
            <div className="space-y-2">
              ...
              <input type="text" value={walletAddressInput} onChange={e=>{setWalletAddressInput(e.target.value);setWalletSaved(false);}} placeholder="EQD... o UQ..."
              ...
              <button onClick={handleSaveWallet} disabled={loading||walletSaved}
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              <input type="number" value={withdrawTon} onChange={e=>setWithdrawTon(e.target.value)} placeholder="0.1" min="0.1" step="0.1"
          ...
          <button onClick={handleWithdraw} disabled={loading||(!isConnected&&!walletSaved)}
